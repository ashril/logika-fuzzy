# Praktikum 3 — Fungsi Keanggotaan dan Fuzzifikasi
## Implementasi Fungsi Keanggotaan Fuzzy dengan Python dan MySQL
...
Pada praktikum sebelumnya telah dipelajari bahwa fungsi keanggotaan fuzzy memetakan nilai input ke dalam derajat keanggotaan $[0, 1]$. Berbagai bentuk kurva dapat digunakan sesuai karakteristik data:

1. **Kurva Segitiga (*Triangular MF*):** Ditentukan oleh 3 parameter $(a, b, c)$ dengan titik puncak tunggal pada $b$.
2. **Kurva Trapesium (*Trapezoidal MF*):** Ditentukan oleh 4 parameter $(a, b, c, d)$ di mana derajat keanggotaan bernilai $1$ secara stabil pada rentang $[b, c]$.
3. **Kurva Gaussian (*Gaussian MF*):** Berbentuk lonceng halus dengan parameter pusat $c$ dan simpangan $\sigma$: $\mu(x) = e^{-\frac{1}{2}\left(\frac{x-c}{\sigma}\right)^2}$.
4. **Kurva Sigmoid (*Sigmoidal MF*):** Kurva transisi bertahap dengan parameter kemiringan $k$ dan titik tengah $c$: $\mu(x) = \frac{1}{1 + e^{-k(x - c)}}$.

Sebagai contoh pemahaman dasar algoritma, mari telaah fungsi keanggotaan **Usia Remaja** berbentuk segitiga:
...
## Tugas Praktikum 3 — Verifikasi Fungsi Keanggotaan
